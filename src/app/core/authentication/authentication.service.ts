...
export class AuthenticationService {
  ...
  private oAuthTokenDetailsStorageKey = 'OAuthTokenDetails';
  ...
  constructor(
    ...
    if (savedCredentials) {
      ...
      if (twoFactorAccessToken) {
        authenticationInterceptor.setTwoFactorAccessToken(
          twoFactorAccessToken.token
        );
      }
    }
  }
  ...
  login(loginContext: LoginContext): Observable<any> {
    ...
    if (environment.oauth.enabled) {
      ...
      // httpParams = httpParams.set('client_id', environment.oauth.clientID);
      // httpParams = httpParams.set('grant_type', 'password');
      // httpParams = httpParams.set('client_secret', environment.oauth.clientSecrect);
      ...
      return this.http
        ...
        .pipe(
          map((credentials: Credentials | any) => {
  ...
  private refreshOAuthAccessToken() {
    ...
    this.http
      ...
      .subscribe((tokenResponse: OAuth2Token | any) => {
        this.storage.setItem(
          this.oAuthTokenDetailsStorageKey,
          JSON.stringify(tokenResponse)
        );
  ...
  logout(): Observable<boolean> {
    ...
    if (twoFactorToken) {
      this.http
        .post('/twofactor/invalidate', { token: twoFactorToken.token })
        .subscribe();
   ...
   */
  resetPassword(passwordDetails: any): any {
    if (this.credentials) {
      return this.http
